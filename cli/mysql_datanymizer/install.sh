...
DATANYMIZER_REPO="${DATANYMIZER}/${DATANYMIZER}"
DATANYMIZER_GITHUB="https://github.com/${DATANYMIZER_REPO}"
DATANYMIZER_RELEASES="${DATANYMIZER_GITHUB}/releases"
DATANYMIZER_CLI="mysql_datanymizer"

usage() {
    require_cmd cat
    this=$1
    cat 1>&2 <<EOF
$this: download binaries for ${DATANYMIZER_CLI}
...
ARGS:
    <tag>       is a tag from ${DATANYMIZER_RELEASES}. If tag is missing, then the latest will be used.
EOF
    exit 2
}
...
main() {
    ...
    _archive_name="${DATANYMIZER_CLI}-${_arch}${_ext}"

    if [ -z "${TAG}" ]; then
        println "The latest version will be installed."
        _url="${DATANYMIZER_RELEASES}/latest/download/${_archive_name}"
    else
        println "Version ${TAG} will be installed"
        _url="${DATANYMIZER_RELEASES}/download/${TAG}/${_archive_name}"
    fi
    ...
    case $_archive_path in
        *.zip)
            ...
            _file_name="${DATANYMIZER_CLI}.exe"
        ;;
        *)  require_cmd tar
            tar -xzf "${_archive_path}" -C "${_temp_dir}"
            _file_name="${DATANYMIZER_CLI}"
        ;;
    esac
